...
def analyze_with_gemini(text, filename):
    ...
    prompt = (
        ...
        "STRUCTURA FACTURII:\n"
        "- Firma prestatoare: SimNettoyage (ignora adresa ei: Macchia Di Cervi, Route de l'Ancienne Voie, San-Gavino-di-Carbini)\n"
        "- 'Facture pour' din antet = CLIENTUL (firma sau persoana care plateste)\n"
        "- Coloana 'Nom' din tabel = numele CASEI sau APARTAMENTULUI\n\n"
        "REGULI OBLIGATORII:\n"
        "1. proprietate = DOAR valoarea din coloana 'Nom' (ex: Antoniotti, Maison Gioia, Appartement A Bala)\n"
        "   IGNORA orice adrese, coduri postale, strazi, orase\n"
        "2. client = valoarea de dupa 'Facture pour' din antet\n"
        "3. CAZUL FORFAIT (pret unitar >= 40 EUR):\n"
        "   - O data = o interventie, qty=1\n"
        "   - Daca sunt N date => N interventii separate, suma = pret_unitar pentru fiecare\n"
        "4. CAZUL ORA (pret unitar < 40 EUR):\n"
        "   - qty total = numarul total de ore (ex: 10 ore)\n"
        "   - Daca sunt N date => N interventii, imparte orele egal: qty_per_interventie = qty_total / N\n"
        "   - suma per interventie = (qty_total / N) * pret_unitar\n"
        "   - Exemplu: 10 ore, 5 date, pret 30 EUR => 5 interventii x 2 ore x 30 EUR = 60 EUR fiecare\n"
        "   - Exemplu: 2 ore, 1 data, pret 26 EUR => 1 interventie x 2 ore x 26 EUR = 52 EUR\n"
        "5. tip_facturare: 'heure' daca pret < 40, 'forfait' daca pret >= 40\n"
        "6. Normalizeaza numele casei: Title Case\n"
        "7. Data format: DD/MM/YYYY (converteste din MM/DD/YYYY)\n\n"
